...
TEST_F(ApplicationConnectedTestSuite, shallHandleSendSmsToUnknownRecipient)
{
    EXPECT_CALL(smsDbMock, getLastMessage());
    objectUnderTest.handleSmsToUnknownRecipient();
}

TEST_F(ApplicationConnectedTestSuite, shallHandleSendMessageSMS)
{
    Sms sms{PHONE_NUMBER, PHONE_NUMBER, "rec message", false, false, false};
    EXPECT_CALL(smsDbMock, addMessage(sms));
    EXPECT_CALL(btsPortMock, sendMessage(sms));
    objectUnderTest.handleSendMessage(sms);
}

TEST_F(ApplicationConnectedTestSuite, shallHandleShowAllMessagesSMS)
{
